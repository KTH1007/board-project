...
public class BoardController {
    ...
    public String boardWritePro(Board board, Model model, MultipartFile file) throws Exception {
        ...
        model.addAttribute("searchUrl", "/board/list"); //redirect와 비슷한 효과
        return "message";
    }
    ...
    public String boardList(Model model, @PageableDefault(page=0, size = 20, sort="id", direction = Sort.Direction.DESC) Pageable pageable,
        ...
        int nowPage = list.getPageable().getPageNumber() + 1; //0부터 시작하기 때문에 1을 더해줌
        int startPage = Math.max(nowPage - 4, 1);  //음수가 나오는 것을 방지하기 위해 Math.max를 사용
